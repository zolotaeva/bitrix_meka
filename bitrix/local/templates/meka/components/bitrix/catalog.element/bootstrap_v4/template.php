...
$buyButtonClassName = in_array('BUY', $arParams['ADD_TO_BASKET_ACTION_PRIMARY']) ? 'btn-buy' : 'btn-link';
$showAddBtn = in_array('ADD', $arParams['ADD_TO_BASKET_ACTION']);
$showButtonClassName = in_array('ADD', $arParams['ADD_TO_BASKET_ACTION_PRIMARY']) ? 'btn-buy' : 'btn-link';
	...
	<div class="product-card__top">
		<div class="row">
			<div class="col-lg-4 col-md-5">
				...
				<div class="product-card__slider product-slider" id="<?=$itemIds['BIG_SLIDER_ID']?>">
					...
					<?=($arParams['IMAGE_RESOLUTION'] === '1by1' ? 'product-item-detail-slider-block-square' : '')?>" data-entity="images-slider-block">
						...
						<div class="product-item-detail-slider-images-container" data-entity="images-container">
								...
								<div class="main-slider swiper">
									<div class="swiper-wrapper">
										...
										if (!empty($actualItem['MORE_PHOTO']))
										{
											...
											{
												?>
												<div class="swiper-slide main-slider__item" data-entity="image" data-id="<?=$photo['ID']?>">
							...
							if ($showSliderControls)
							{
								...
								{
									?>
									<div class="product-item-detail-slider-controls-block product-slider__thumbs" id="<?=$itemIds['SLIDER_CONT_ID']?>">
											...
											<div class="swiper-wrapper">
												...
												if (!empty($actualItem['MORE_PHOTO']))
												{
													foreach ($actualItem['MORE_PHOTO'] as $key => $photo)
													{
														...
														<div class="swiper-slide thumbs-slider__item" data-entity="slider-control" data-value="<?=$photo['ID']?>">
															<img src="<?=$photo['SRC']?>">
														</div>
														<?php
													}
			...
			<div class="col-lg-5 col-md-7">
				<div class="product-card__info">
					...
					{
						?>
						<h1 class="page__title title pb-60"><?=$name?></h1>
						<?php
					}?>
			...
			?>
				<div class="product-card__props product-props">
					<div class="product-props__title">Характеристики</div>
											...
											<div class="product-props__list">
												...
												if (!empty($mainBlockProperties))
												{
													?>
														...
														{
															?>
															<div class="product-props__item product-item-detail-properties-item">
																<div class="product-props__name product-item-detail-properties-name text-muted"><?=$property['NAME']?></div>
																<div class="product-item-detail-properties-value product-props__text"><?=(is_array($property['DISPLAY_VALUE'])
					...
					<div class="product-props__more">
						<a href="#tab-content-2" class="btn btn-link product-props__btn">Все характеристики</a>
					</div>
				</div>
			</div>
		</div>
		<div class="col-lg-3">
			...
			<div class="product-card__controls product-controls">
					...
					switch ($blockName)
					{
						...
						case 'price':
							...
							<?if($actualItem['CAN_BUY']){?>
								<div class="product-controls__title">Стоимость</div>
							<?}?>
								...
								{
									?>
									<div class="product-item-detail-economy-price product-controls__mark"
								...
								<div class="product-controls__price">
									...
									<span class="product-item-detail-price-current current-price" id="<?=$itemIds['PRICE_ID']?>"><?=$price['PRINT_RATIO_PRICE']?></span>
								<?php
								if ($arParams['SHOW_OLD_PRICE'] === 'Y')
								{
									?>
									
									<span class="product-item-detail-price-old old-price"
						...
						case 'quantity':
							...
							{
								?>
								<div class="product-controls__pay">
									<div <?= (!$actualItem['CAN_BUY'] ? ' style="display: none;"' : '') ?> data-entity="quantity-block">
										...
										*/}
										...
										<div class="product-item-amount">
											<div class="product-item-amount-field-container product-controls__quantity quantity">

												<button type="button" class="product-item-amount-field-btn-minus no-select btn quantity__button js-down" id="<?=$itemIds['QUANTITY_DOWN_ID']?>">-</button>
												<input class="product-item-amount-field quantity__input" id="<?=$itemIds['QUANTITY_ID']?>" type="number" value="<?=$price['MIN_QUANTITY']?>">
												<button type="button" class="product-item-amount-field-btn-plus no-select btn quantity__button js-up" id="<?=$itemIds['QUANTITY_UP_ID']?>">+</button>
						...
						case 'buttons':
							?>
								<div data-entity="main-button-container" class="product-controls__btn">
									<div id="<?=$itemIds['BASKET_ACTIONS_ID']?>" style="display: <?=($actualItem['CAN_BUY'] ? '' : 'none')?>;">
										...
										if ($showAddBtn)
										{
											?>
												...
												<a class="btn <?=$showButtonClassName?> product-item-detail-buy-button btn-request"
													id="<?=$itemIds['ADD_BASKET_LINK']?>"
													href="javascript:void(0);" data-bs-toggle="modal" data-bs-target="#order" data-name="<?=$name?>" data-link="https://meka.akticom-dev.ru<?=$arResult['DETAIL_PAGE_URL']?>">
										...
										if ($showBuyBtn)
										{
											?>
												...
												<a class="btn <?=$buyButtonClassName?> product-item-detail-buy-button btn-request"
													id="<?=$itemIds['BUY_LINK']?>"
													href="javascript:void(0);" data-bs-toggle="modal" data-bs-target="#order" data-name="<?=$name?>" data-link="https://meka.akticom-dev.ru<?=$arResult['DETAIL_PAGE_URL']?>">
								...
								?>
									...
									$APPLICATION->IncludeComponent(
										...
										array(
											...
											'BUTTON_CLASS' => 'product-controls__title center',
							...
							*/}
							...
							<div id="<?=$itemIds['NOT_AVAILABLE_MESS']?>" style="display: <?=(!$actualItem['CAN_BUY'] ? '' : 'none')?>;">
								<div class="product-controls__title center"><?=$arParams['~MESS_BTN_SUBSCRIBE']?></div>
								<a class="btn btn-buy btn-request w-100" href="javascript:void(0)" rel="nofollow" data-bs-toggle="modal" data-bs-target="#order" data-name="<?=$name?>" data-link="https://meka.akticom-dev.ru<?=$arResult['DETAIL_PAGE_URL']?>"><?=$arParams['MESS_NOT_AVAILABLE']?></a>
							</div>
				...
				<?if($actualItem['CAN_BUY']){?>
					<div class="product-controls__action">
						<?
						if($arResult['PREVIEW_PICTURE']['ID'] != 0){
						$imgMin = CFile::ResizeImageGet($arResult['PREVIEW_PICTURE']['ID'], array('width'=>160, 'height'=>130), BX_RESIZE_IMAGE_EXACT, true);
						$imgMin = $imgMin['src'];
						}else{
							$imgMin = $arResult['MORE_PHOTO'][0]['SRC'];
						}
						...
						//{
							...
							<label class="product-controls__compare js-compare" id="<?=$itemIds['COMPARE_LINK']?>">
								<input type="checkbox" data-entity="compare-checkbox">
								<span data-entity="compare-title"><?=$arParams['MESS_BTN_COMPARE']?></span>
							</label>
						
							<label class="product-controls__favorite js-favorite <?=$inFavorite?>">
								<input type="checkbox" data-id="<?=$arResult['ID']?>" data-img="<?=$imgMin?>" data-name="<?=$arResult['NAME']?>" data-link="<?=$arResult['DETAIL_PAGE_URL']?>">
...
*/?>
		...
		<div class="product-card__tabs product-tabs">
			<div class="product-tabs__nav" id="<?=$itemIds['TABS_ID']?>">
				<?php
				if ($showDescription)
				{
					?>
					<a href="#tab-content-1" class="product-tabs__link active" data-entity="tab" data-value="description">Описание</a>
				...
				{
					?>
					<a href="#tab-content-2" class="product-tabs__link" data-entity="tab" data-value="properties">Характеристики</a>

					<? if($arResult['PROPERTIES']['PASPORT']['VALUE'] || $arResult['PROPERTIES']['DOCUMENTS']['VALUE']){?>
						<a href="#tab-content-3" class="product-tabs__link">Документация</a>
					<?}?>


					<? if($arResult['PROPERTIES']['VIDEO_MONTAG']['VALUE'] || $arResult['PROPERTIES']['VIDEO_OTHER']['VALUE']){?>
						<a href="#tab-content-4" class="product-tabs__link">Видео монтажа</a>
					<?}?>
					<?php
				}
			...
			<div class="product-tabs__content" id="<?=$itemIds['TAB_CONTAINERS_ID']?>">
					...
					if ($showDescription)
					{
						...
						<div class="product-tabs__item active"
							...
							itemprop="description" id="tab-content-1">
							<div class="product-tabs__description">
								<div class="product-tabs__title">Описание</div>
								<?php
								if (
								...
								if ($arResult['PROPERTIES']['MARKETING_DESCRIPTION']['VALUE'] != '')
								{
									echo $arResult['PROPERTIES']['MARKETING_DESCRIPTION']['~VALUE']['TEXT'];
								}
					...
					{
						?>
						<div class="product-tabs__item" data-entity="tab-container" data-value="properties" id="tab-content-2">
							<?php
							if (!empty($arResult['DISPLAY_PROPERTIES']))
							{
								?>
								<div class="product-tabs__props">
									<div class="product-tabs__title">Характеристики</div>
									<div class="product-props__list">
									<?php
									foreach ($arResult['DISPLAY_PROPERTIES'] as $property)
									{
										...
										<div class="product-props__item">
											<div class="product-props__name"><?=$property['NAME']?></div>
											
											<div class="product-props__text"><?=(
					...
					<? if($arResult['PROPERTIES']['PASPORT']['VALUE'] || $arResult['PROPERTIES']['DOCUMENTS']['VALUE']){?>
						...
						<div class="product-tabs__item" id="tab-content-3">
							<div class="product-tabs__docs">
								<div class="product-tabs__title">Документы</div>
								<div class="product-docs">

									<?if($arResult['PROPERTIES']['PASPORT']['VALUE']){


										$arrFile = CFile::GetFileArray($arResult['PROPERTIES']['PASPORT']['VALUE']);
										...
										<a href="<?=$fileLink?>" target="_blank" class="product-docs__item">
											<span class="product-docs__title">Технический паспорт изделия</span>
											<span class="product-docs__type"><?=$fileType?></span>
											<span class="product-docs__size"><?=$fileSize?></span>
										</a>
									...
									<?if($arResult['PROPERTIES']['DOCUMENTS']['VALUE']){
										...
										$arSelect = Array("ID", "IBLOCK_ID", "NAME", "PROPERTY_DOCUMENT");
										$arFilter = Array("IBLOCK_ID"=>18, "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y", "ID"=>$arResult['PROPERTIES']['DOCUMENTS']['VALUE']);
										...
										foreach ($arDocs as $key => $doc) {
											$arrFile = CFile::GetFileArray($doc['PROPERTY_DOCUMENT_VALUE']);
											$file = CFile::CheckImageFile($arrFile);
											$fileType = explode('/', $arrFile["CONTENT_TYPE"])[1];
											$fileType = strtoupper($fileType);
											if(strpos($fileType, 'EXCEL') !== false ){
											...
											<a href="<?=$fileLink?>" target="_blank" class="product-docs__item">
												<span class="product-docs__title"><?=$doc['NAME']?></span>
												<span class="product-docs__type"><?=$fileType?></span>
												<span class="product-docs__size"><?=$fileSize?></span>
											</a>
						...
						<? if($arResult['PROPERTIES']['VIDEO_MONTAG']['VALUE'] || $arResult['PROPERTIES']['VIDEO_OTHER']['VALUE']){?>
							...
							<div class="product-tabs__item" data-entity="tab-container" id="tab-content-4">
								<div class="product-tabs__title">Монтаж</div>
								<div class="row">
									<div class="col-lg-11">
										<div class="row">
											<? if($arResult['PROPERTIES']['VIDEO_MONTAG']['VALUE']){?>
											<div class="col-md-6">
												<div class="video-item">
													<iframe src="<?=$arResult['PROPERTIES']['VIDEO_MONTAG']['VALUE']?>" width="100%" height="300" title="Монтаж" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
												</div>
											</div>
											<?}?>

											<? if($arResult['PROPERTIES']['VIDEO_OTHER']['VALUE']){?>
											<div class="col-md-6">
												<div class="video-item">
													<iframe src="<?=$arResult['PROPERTIES']['VIDEO_OTHER']['VALUE']?>" width="100%" height="300" title="Монтаж" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
												</div>
											</div>
											<?}?>
										</div>
