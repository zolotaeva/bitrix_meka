...
<?}?>
	...
	foreach ($arResult["QUESTIONS"] as $FIELD_SID => $arQuestion)
	{
		...
		else
		{
			...
			if($FIELD_SID == 'SIMPLE_QUESTION_644'){
				$mainclass = 'class="'.$FIELD_SID .' form-control mask" placeholder="'.$arQuestion["CAPTION"].'"';
			}

			if($FIELD_SID == 'SIMPLE_QUESTION_406'){
				$mainclass = 'class="'.$FIELD_SID .' hide" placeholder="'.$arQuestion["CAPTION"].'" value=""';
			}
...
?>
	<div class="form__agree mt-4">
		<input type="checkbox" name="" id="agreeOrder" class="custom-checkbox">
		<label for="agreeOrder">Согласен с <a href="/policy/" target="_blank">политикой обработки персональных данных</a></label>
	</div>
	<div class="form__btn">
...
<script>
	...
	let productName = document.querySelector('.SIMPLE_QUESTION_183');
	...
	let agreeOrder = document.getElementById('agreeOrder');
	if(!agreeOrder.checked){
		document.querySelector('#order .btn').disabled = true;
	}

	agreeOrder.addEventListener('change', ()=>{
		if(!agreeOrder.checked){
			document.querySelector('#order .btn').disabled = true;
			agreeOrder.nextElementSibling.classList.add('error-check');
		}else{
			document.querySelector('#order .btn').disabled = false;
			agreeOrder.nextElementSibling.classList.remove('error-check');
		}
	})
})
</script>