...
<div class="product">
	...
	{
		$compareItems = $_SESSION['CATALOG_COMPARE_LIST'][$item['IBLOCK_ID']]['ITEMS'];
		$inCompare = isset($compareItems[$item['ID']]);
		?>

					<label id="<?=$itemIds['COMPARE_LINK']?>" class="product__compare js-compare <?= $inCompare ? 'in-compare' : '' ?>">
						<input type="checkbox" data-entity="compare-checkbox" <?= $inCompare ? 'checked' : '' ?>>
	...
	if($item['PREVIEW_PICTURE']['ID'] != 0){
		$imgMin = CFile::ResizeImageGet($item['PREVIEW_PICTURE']['ID'], array('width'=>160, 'height'=>130), BX_RESIZE_IMAGE_EXACT, true);
		$imgMin = $imgMin['src'];
	}else{
		$imgMin = $item['PREVIEW_PICTURE']['SRC'];
	}
	...
	<label class="product__favorite js-favorite <?=$inFavorite?>">
		<input type="checkbox" data-id="<?=$item['ID']?>" data-img="<?=$imgMin?>" data-name="<?=$item['NAME']?>" data-link="<?=$item['DETAIL_PAGE_URL']?>">
		<span></span>
	</label>
	...
	<? if ($itemHasDetailUrl): ?>
		<a class="product-item-image-wrapper product__image" href="<?=$item['DETAIL_PAGE_URL']?>" title="<?=$imgTitle?>"
		data-entity="image-wrapper">
	<? else: ?>
		<span class="product__image product-item-image-wrapper" data-entity="image-wrapper">
	<? endif; ?>
		...
		<!-- class="product-item-image-original" -->
		 <?
		 $img = CFile::ResizeImageGet($item['PREVIEW_PICTURE']['ID'], array('width'=>268, 'height'=>268), BX_RESIZE_IMAGE_PROPORTIONAL, true);?>
		<span class="product-item-image-original" id="<?=$itemIds['PICT']?>">
			<img src="<?=$img['src']?>" alt="<?=$productTitle?>">
		</span>
...
<div class="product__info">
	...
	<a href="<?=$item['DETAIL_PAGE_URL']?>" class="product__title"><?=$productTitle?></a>
				...
				case 'props':
					if (!$haveOffers)
					{
						if (!empty($item['DISPLAY_PROPERTIES']))
						{
							...
							<div class="product__option option" data-entity="props-block">
									...
									foreach ($item['DISPLAY_PROPERTIES'] as $code => $displayProperty)
									{
										...
										<? if ($displayProperty['ID'] == 60 || $displayProperty['ID'] == 44){
											$class = ' w-100';
										}else{
											$class = '';
										}?>
										<div class="option__item<?=$class?>">
											<?
											$displayProperty['NAME'] = preg_replace('/\s*\(мм\)\s*/', '', $displayProperty['NAME']);?>
											<div class="option__name"><?=$displayProperty['NAME']?></div>
...
<a href="<?=$item['DETAIL_PAGE_URL']?>" class="btn product__btn btn-primary">Подробнее</a>
</div>